...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query("SELECT u FROM User u WHERE u.id IS ?1 AND (?3 = TRUE OR u.id IN ?2)")
    Optional<User> findById(Long id,List<Long> allowedIds, boolean ignoreAuth);

    @Query("SELECT u FROM User u WHERE u.personalData.firstname IS ?1 AND (?3 = TRUE OR u.id IN ?2)")
    List<User> findByPersonalData_Firstname(String firstname,List<Long> allowedIds, boolean ignoreAuth);

    @Query("SELECT u FROM User u WHERE u.personalData.lastname IS ?1 AND (?3 = TRUE OR u.id IN ?2)")
    List<User> findByPersonalData_Lastname(String lastname,List<Long> allowedIds, boolean ignoreAuth);

    @Query("SELECT u FROM User u WHERE u.personalData.firstname IS ?1 AND u.personalData.lastname IS ?2 AND " +
            "(?4 = TRUE OR u.id IN ?3)")
